...
3. **Threshold value `T`:** default **`T = 8 m`**. Rationale: the earlier default of 3 m treated source provenance as unknown and picked a value at the top of the *barometric* band (§5.2) as a supposedly-conservative compromise. That premise does not hold: provenance is not actually unknown for this library's inputs. GPX always encodes GPS-derived altitude, and FIT altitude is GPS-derived too unless the device supplies a barometric `session.total_ascent` — which step 1 already prefers when present, so by the time step 2's filter runs, the stream in front of it is GPS-derived essentially every time. A 3 m threshold applied to GPS altitude noise (±3-5 m per fix, §5.1) barely filters anything. `T = 8 m` instead sits inside the GPS-derived band the authoritative sources actually recommend for this case — Strava's ~10 m for non-barometric activities, GPS Visualizer's 6-9 m — deliberately toward the middle-to-upper end of that range rather than its edge. Where the source is known to carry a barometric altimeter, a smaller `T` (toward the 2-3 m barometric figure of §5.2) is more faithful; expose `T` as a parameter so this is tunable rather than hard-coded. Document the default and its source.
4. **Known limitation — trailing unconfirmed rise.** Hysteresis credits a climb only once the cumulative rise from the last confirmed reference point clears `T`; whatever portion of a climb has not yet cleared `T` when the point stream ends is never credited, no matter how real it is. Example: a monotonic 10 m → 15 m climb recorded in 1 m steps (`10, 11, 12, 13, 14, 15`) reports `3 m` of gain at `T = 3` — confirmed once the cumulative rise from `ref = 10` reaches `13`; the trailing `13 → 15` rise of 2 m never re-clears the band and is dropped — and `0 m` of gain at `T = 8`, because the full 5 m rise never clears the threshold at all, so the *entire* climb is the unconfirmed trailing portion. This is inherent to any single-pass hysteresis filter, not a bug: crediting an in-progress, still-ambiguous rise would reopen exactly the noise-integration problem `T` exists to reject.
5. **Moving-average alternative (rejected as the primary method).** Smoothing elevation with a moving average *before* differencing also reduces noise (and is what Strava does *in addition* to its threshold). It is rejected as the sole mechanism because a moving average alone still accumulates small residual positive deltas across the whole track (see the worked example: it yields 2.67 m of "gain" on a noise-only-plus-5 m signal, better than raw but with no principled zero). Hysteresis has a clean noise-rejection floor. The maintainer may layer a moving average *before* the hysteresis pass for extra robustness; that is compatible with this spec.
6. **`elevationLossM`** must use the **same** filter (accumulate confirmed descents), so gain and loss are symmetric and both denoised. Today loss has the identical raw-delta defect.
